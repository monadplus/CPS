...
public:
  ...
  // Pick the smallest value in r (the one with less possible solutions)
  // and in case of a tie, return the one most centered
  int value_aux(IntVar x, int i) const {
    ...
    for (IntVarValues k(x); k( ) ; ++k) {
      ...
      else if (/*In case of a tie*/mins == int(r[j].size()) and /*most centered*/abs(j - n/2.) < abs(minj - n/2.)) {
    	minj = j;
      }
    }
    return minj;
