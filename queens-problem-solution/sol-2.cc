...
public:
  Queens(int nn) : n(nn), c(*this, n, 0, n-1) {
    ...
    // At most 1 queen per column, asc., desc. diagonal.

    // For diagonals of the form i+j = k,
    // the k corresponding to (i, ci) is i + ci.
    // So queens (i, ci) and (j, cj) must satisfy that i + ci != j + cj, i.e., ci + i != cj + j
